...
```{julia}
target_location = "Baylor County"  # <1>
target_lon = 34.16           # <2>
target_lat = -99.68              # <3>
```
...
μ = 3    # Location parameter - shifts the distribution left/right #it was 4 
σ = 1.5    # Scale parameter - controls spread (must be > 0) 
##1.3 to begin with 
ξ = .1   # Shape parameter - controls tail behavior
##0.15 to begin with 
...
function plot_empirical_vs_theoretical(station_data, gev_dist, station_info)
	...
	emp_levels = ustrip.(u"inch", emp_levels)
...
Station name: Bronwsville 
ID: 79-0043

With the exception of strong postive outlier in the late 1960's and the early 1980's, over time, the annual maximum rainfall increases (frequency of higher maximum rainfall increases following 1980's). 
...
Station name: Mercedes 6 SSE
ID: 41-5836
When observing the rainfall data in the 4 stations nearest to Lubbock, I noted that, with the exception of some outlier (ex: large outlier in 2023), the station data trends began to decrease as the data approaches 2025. 
...
- Final: $\mu = 3$ $\sigma =1.5$, $\xi = 0.1$

**Parameter Effects:**

**What happened when you increased/decreased $\mu$ (location parameter)?**

When the location parameter is increased, the GEV Model shifted upwards. When the location parameter was decreased,  the GEV Model shifted downwards. 
...
As mu values moved farther away from the ideal mu parameter (mu=3 or 4), the GEV curve did a worse job at fitting the red data points. 
...
As sigma values approach 0, the curve becomes flatter, and the spread of return levels decreases. As sigma values increase, the curve becomes steeper, and the spread of return values increase. 
...
Lower xi values the curve flattened and tended to fit extremes in the leftward part of the distribution.In contrast, higher xi values fit the outlier on the right side of the distribition better. 


**Final Assessment:**

**How well does your best-fit GEV distribution represent the closest station's data?**
My best-fit GEV distribtuon represents the closest station data well. The red dots follow the blue curve closely,  with exceptions at return periods of around 60 and 135 years. 
...
It was interesting to see the GEV curve take the shape of different functions as you changed the parameters. When I was messing with the parameters, I saw the GEV curve look like a square root function, 1/x, and more. 